...
void nonMaximaSuprresionCpu(const TYPE* src, int srcRows, int srcCols, int srcStep, int srcChannels,
	...
	//std::sort(begin(random), end(random), by_cornerResponse());
	//std::sort(points, points+N-1, by_cornerResponse);

        // Get top points, given by the percentage
        int numberTopPoints = srcCols * srcRows * percentage;
        cornerXY topPoints[N];

        int i=0;
        while(i < numberTopPoints) {
            if(i == N) 
                break;
...
void nonMaximaSuprresion(const TYPE* src, int srcRows, int srcCols, int srcStep, int srcChannels,
                    TYPE* dest, int destRows, int destCols, int destStep, int destChannels,
		    cornerXY* pointsDSA, float percentage, int filterRange, int suppressionRadius){
...
#pragma ss config
	...
	#pragma ss stream nonblock 
        ...
        // Sort pointsDSA by corner Response
	//std::sort(begin(random), end(random), by_cornerResponse());
	//std::sort(pointsDSA, pointsDSA+N-1, by_cornerResponse);

        // Get top pointsDSA, given by the percentage
        int numberTopPoints = srcCols * srcRows * percentage;
        cornerXY topPoints[N];
        ...
        while(i < numberTopPoints) {
            ...
            // Check if point marked in maximaSuppression matrix
            if(dest[(pointsDSA[i].x)*destStep + pointsDSA[i].y] == 0) {
                for (int r = -suppressionRadius; r <= suppressionRadius; r++) {
                    for (int c = -suppressionRadius; c <= suppressionRadius; c++) {
                        int sx = pointsDSA[i].x+c;
                        int sy = pointsDSA[i].y+r;
                        ...
                        int index = (pointsDSA[i].x+r)*destStep + pointsDSA[i].y+c;
                        if(index>=0 && ((pointsDSA[i].x+r)*destStep)>=0 && (pointsDSA[i].y+c)>=0
                            && ((pointsDSA[i].x+r)*destStep)<destRows && (pointsDSA[i].y+c)<destCols) dest[index] = 1;
                    }
                }

                // Convert back to original image coordinate system
                pointsDSA[i].x += 1 + filterRange;
                pointsDSA[i].y += 1 + filterRange;
                topPoints[i] = pointsDSA[i];
            }
