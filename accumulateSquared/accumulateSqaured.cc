...
#define U 8 
#endif
void accumulateSquaredCpu(const TYPE* image1, int imageRows, int imageCols, int imageStep, int imageChannels,
...
void accumulateSquared(TYPE src[N], TYPE dest[N]){
	...
	//GRAY
          #pragma ss stream 
	        ...
	        for (j=0; j<cols; ++j){
                  dest[i*cols + j] += src[i*cols + j] * src[i*cols + j];
		  //temp1 = src[i*cols + j];
		  //temp1 = temp1*temp1;
                  //dest[i*cols + j] += temp1;
	        }
...
int main() {
  ...
  accumulateSquaredCpu(image, row_size, col_size, col_size, C, dest1, row_size, col_size, col_size, C);

  //cache warmup
  for(int i=0;i<CACHE_WARMUP;++i)
        //accumulateSquared(image, row_size, col_size, col_size, C, dest, row_size, col_size, col_size, C);
        accumulateSquared(image, dest);
